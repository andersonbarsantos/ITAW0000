...
namespace ITAW000.Repository
{
    public class SistemaRepository : AbstractRepository<Sistema, int>
    {
        public override void Delete(Sistema entity)
        {
            ...
            {
                string sql = "DELETE Sistema_tb Where Id=@Id";
        ...
        public override Sistema GetById(int id)
        {
            using (var conn = new SqlConnection(StringConnection))
            {
                string sql = "Select * FROM Sistema_tb WHERE Id=@Id";
                SqlCommand cmd = new SqlCommand(sql, conn);
                cmd.Parameters.AddWithValue("@Id", id);
                Sistema Sistema = null;
                try
        ...
        public override void Add(Sistema entity)
        {
            using (var conn = new SqlConnection(StringConnection))
            {
                string sql = "INSERT INTO Sistema_tb (Nome, Descricao, DtInclusao, DtAlteracao, Usuario) " +
        ...
        public override void Update(Sistema entity)
        {
            using (var conn = new SqlConnection(StringConnection))
            {
                string sql = "UPDATE Sistema_tb SET Nome=@Nome, Descricao=@Descricao, DtAlteracao=@DtAlteracao, Usuario=@Usuario Where Id=@Id";
                SqlCommand cmd = new SqlCommand(sql, conn);
                cmd.Parameters.AddWithValue("@Id", entity.IdSistema);
                cmd.Parameters.AddWithValue("@Nome", entity.Nome);
                cmd.Parameters.AddWithValue("@Descricao", entity.Descricao);
